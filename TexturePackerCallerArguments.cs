...
namespace TextureBatchPacker
{
	partial class TexturePackerCaller
	{
		private string GetTexturePackerArguments_PNG_8888(ConvertionParameters parameters)
		{
			...
			if (parameters.NoTrim)
			{
				argument = string.Format(
					"--format cocos2d --data \"{0}\" --texture-format png --png-opt-level 0 --dpi 72 --opt RGBA8888 --max-size 16384 --size-constraints AnySize --force-word-aligned --scale {1} --scale-mode Smooth --algorithm MaxRects --maxrects-heuristics Best --pack-mode Best --border-padding 0 --shape-padding 2 --inner-padding 0 --extrude 0 --disable-rotation --trim-mode None \"{2}\"",
					plistFullPath,
					parameters.Scale,
		...
		private string GetTexturePackerArguments_PNG_4444(ConvertionParameters parameters)
		{
			...
			{
				argument = string.Format(
					"--format cocos2d --data \"{0}\" --texture-format png --png-opt-level 0 --dpi 72 --opt RGBA4444 --dither-fs-alpha --max-size 16384 --size-constraints AnySize --force-word-aligned --scale {1} --scale-mode Smooth --algorithm MaxRects --maxrects-heuristics Best --pack-mode Best --border-padding 0 --shape-padding 2 --inner-padding 0 --extrude 0 --disable-rotation --trim-mode None \"{2}\"",
					plistFullPath,
					parameters.Scale,
					parameters.SrcDir.FullName);
			}
		...
		private string GetTexturePackerArguments_PNG_565(ConvertionParameters parameters)
		{
			...
			{
				argument = string.Format(
					"--format cocos2d --data \"{0}\" --texture-format png --png-opt-level 0 --dpi 72 --opt RGB565 --dither-fs --max-size 16384 --size-constraints AnySize --force-word-aligned --scale {1} --scale-mode Smooth --algorithm MaxRects --maxrects-heuristics Best --pack-mode Best --border-padding 0 --shape-padding 2 --inner-padding 0 --extrude 0 --disable-rotation --trim-mode None \"{2}\"",
					plistFullPath,
					parameters.Scale,
					parameters.SrcDir.FullName);
			}
		...
		private string GetTexturePackerArguments(ConvertionParameters parameters)
		{
			switch (parameters.TextureFormat)
			{
				...
				default:
					if (parameters.TextureQuality != TEXTURE_QUALITY.LOW)
					{
