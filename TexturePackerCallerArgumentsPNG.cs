...
namespace TextureBatchPacker
{
	partial class TexturePackerCaller
	{
		...
		private string GetTexturePackerArguments_PNG_888(ConvertionParameters parameters)
		{
			...
			{
				argument = string.Format(
					"--format cocos2d --data \"{0}\" --texture-format png --png-opt-level 1 --dpi 72 --opt RGBA888 --max-size 16384 --size-constraints AnySize --force-word-aligned --scale {1} --scale-mode Smooth --algorithm MaxRects --maxrects-heuristics Best --pack-mode Best --border-padding 0 --shape-padding 2 --inner-padding 0 --extrude 0 --disable-rotation --trim-mode None \"{2}\"",
					plistFullPath,
					parameters.Scale,
					parameters.SrcDir.FullName);
			}
