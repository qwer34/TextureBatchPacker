...
namespace TextureBatchPacker
{
	class TextureBatchPackerArgs
	{
		...
		[ArgRequired(PromptIfMissing = false), ArgExistingDirectory, ArgShortcut("i"), ArgDescription("The input directory.")]
		public string InputDirectory { get; set; }

		[ArgRequired(PromptIfMissing = false), ArgExistingDirectory, ArgShortcut("o"), ArgDescription("The output directory.")]
	...
	class Program
	{
		static int Main(string[] args)
		{
			...
			try
			{
				...
				//processPngQuant.StartInfo.RedirectStandardInput = true; //标准输入
				...
				//Console.WriteLine("打包 {0}", NameWithoutExt);
